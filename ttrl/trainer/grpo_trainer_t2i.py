...
from ttrl.rewards.t2i import TTRLT2ICycleCSReward
...
class JanusProTTGRPOTrainer(Trainer):
    def __init__(
        ...
        for i, reward_func in enumerate(reward_funcs):
            if isinstance(reward_func, str) and 'TTRL_CycleCS' in reward_func:
                reward_funcs[i] = TTRLT2ICycleCSReward(self.task_args)
        ...
        # ******************* Training arguments *******************
        self.max_prompt_length = args.max_prompt_length  # prompt
        ...
        # including those within a grad accum cycle
        self._step = 0
        # Buffer the batch to reuse generated outputs across multiple updates. For more details,
        #  see `_get_train_sampler` and `_prepare_inputs`.
        ...
        if self.ref_model is not None:
            del self.ref_model.vision_model
            del self.ref_model.aligner
        ...
        for i, reward_func in enumerate(self.reward_funcs):
            ...
            elif isinstance(reward_func, TTRLT2ICycleCSReward):
    ...
    def init_trainable_parameters(model):
        ...
        for param in model.gen_vision_model.parameters():
            param.requires_grad = False
        # del model.gen_vision_model.encoder
    ...
    def _get_per_token_logps(
            self, model,
            input_ids, attention_mask, discrete_img_ids, logits_to_keep, batch_size=None
    ):
        discrete_img_ids = discrete_img_ids.to(input_ids.dtype)
        ...
        for i in range(0, input_ids.size(0), batch_size):
            ...
            discrete_img_ids_batch = discrete_img_ids[i: i + batch_size]

            logits = model(
                t2i_input_ids=input_ids_batch,
                t2i_attention_mask=attention_mask_batch,
                t2i_discrete_img_ids=discrete_img_ids_batch,
                t2i_logits_to_keep=logits_to_keep,
                task="generation"
            ).logits  # (B, L, V)

            # Divide logits by sampling temperature.
            logits = logits / self.temperature
            logps = selective_log_softmax(logits, discrete_img_ids)
            all_logps.append(logps)

        return torch.cat(all_logps, dim=0)

    def wrap_t2i_prompt(self, inputs, device):
        prompts = []
        for xp in inputs:
    ...
    def _generate_and_score_completions(
        ...
        prompt_inputs, prompts = self.wrap_t2i_prompt(inputs, device)
        ...
        with unwrap_model_for_generation(
            ...
            # discrete image IDs, generated images
            completion_ids, completions = unwrapped_model.t2i_generate(
                input_ids=prompt_ids, attention_mask=prompt_mask, **self.generation_kwargs
            )

        completion_mask = torch.ones_like(completion_ids).int()
        ...
        with torch.inference_mode():
            if self.num_iterations > 1 or self.args.steps_per_generation > self.args.gradient_accumulation_steps:
                old_per_token_logps = self._get_per_token_logps(
                    ...
                    discrete_img_ids=completion_ids,
        ...
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            if reward_func.__name__ == "TTRL_CycleCS":
                ...
                output_reward_func = reward_func(
                    completions=completions,
                    prompts=prompts,
                    problems=[ann['problems'] for ann in inputs],
                    answers=[ann['answers'] for ann in inputs],
                    num_generations=self.num_generations,
                    mmgpt=self.model,
                    processing_class=self.processing_class,
                )
    ...
    def compute_loss(
        ...
        per_token_logps = self._get_per_token_logps(
            ...
            discrete_img_ids=inputs["discrete_img_ids"],
            logits_to_keep=logits_to_keep,
        )
